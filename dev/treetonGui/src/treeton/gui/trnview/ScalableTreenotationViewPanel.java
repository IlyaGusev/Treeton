...
public class ScalableTreenotationViewPanel extends TreenotationViewPanelAbstract implements ActionListener, AdjustmentListener {
    ...
    ScalableTreenotationViewPanel(TrnType[] tp, Fraction _from, Fraction _to, Fraction _focus, TreenotationStorage trns, String doc, int _curPos, HashMap<String, TrnLabelGenerator> labelGenerators) {
        super();
        treenotations = trns;
        dp = null;
        verScroll = null;
...
//        return focus;
        throw new UnsupportedOperationException(); // todo: do we really want this method and what must it return?
    }
    ...
    protected void init() {
        dp = new ScalableTreenotationViewPanel.DrawingPane();
        ...
        scaleButton.addMouseListener(scaleButton);
...
//            cur = from + e.getValue() * horStep;
            cur = e.getValue() * horStep;
//            focus = cur + fake; // todo: if we make cur==focus, this code isn't correct; do we really need focus, maybe rid of it?
//            fake = focus + fake;
//            if (fake > to) { //todo: can we remove this code?
...
//            }
    ...
    public void componentResized(ComponentEvent e) {
        ...
        if (dp.initialized) {
            ...
            gridButton.setBounds(dim.width - vsDim.width, dim.height - (int) Math.round(hsDim.height * 0.4), (int) Math.round(vsDim.width * 0.5), (int) Math.round(hsDim.height * 0.4));
            scaleButton.setBounds(dim.width - (int) Math.round(vsDim.width * 0.5), dim.height - (int) Math.round(hsDim.height * 0.4), (int) Math.round(vsDim.width * 0.5), (int) Math.round(hsDim.height * 0.4));
    ...
    public void recountHorScroll() {
        if (focus != null) {
            ...
            fake = (double) dim.width / (double) (dp.cellWidth * 2);
            cur = focus - fake;
            cur = cur < from ? from : Math.floor(cur / horStep) * horStep;
        }
//        Dimension dim = dp.getSize();
//
//        fake = (double)dim.width / (double)(dp.cellWidth*2);
...
//        fake = (cur - from)/horStep;
        horScroll.setValues((int) (cur / horStep), 1, (int) (from / horStep), (int) (to / horStep));
    }
    ...
    public void fitToView() {
        if (!dp.initialized) {
            ...
            while (width * c <= pixelWidth) { // todo: this works only for non-scaled coordinates
                c += 2;
            }
            c -= 2;
    ...
    public void setCursorPosition(int _curPos) {
        if (dp != null) {
            ...
            if (oldPos >= (int) Math.floor(cur) && oldPos <= (int) Math.floor(fake) || // this is correct only when scale is >=1
    ...
    class DrawingPane extends JComponent implements MouseListener, MouseMotionListener {
        ...
        CoordConverter[] coordConverters = new CoordConverter[]{new CoordConverterNonScaledImpl(), new CoordConverterMaxLabelLengthImpl()};
        int coordConverterNum = 0;
        CoordConverter coord = coordConverters.length == 0 ? new CoordConverterNonScaledImpl() : coordConverters[0];
        ...
        public void mouseDragged(MouseEvent e) {
            ...
            if (px > dp.getVisibleRect().getMaxX()) {
                ...
                horScroll.updateUI();
                px = (int) dp.getVisibleRect().getMaxX();
            }

            if (px < dp.getVisibleRect().getMinX()) {
                horScroll.setValue(horScroll.getValue() - 1);
                horScroll.updateUI();
                px = (int) dp.getVisibleRect().getMinX();
            }


            int q = (int) (coord.increaseStoragePos(cur, (double) px / cellWidth));
        ...
        private void fillGrid() throws TreetonModelException {
            ...
            // правая граница отображаемой области в символьных координатах при отсутствии масштабирования
            // в случае масштабирования может вылезать за правую границу отображаемой области, но не должна быть левее
            // (иначе grid не хватит на всю отображаемую область) => масштаб должен быть >=1
                        ...
                        private Treenotation findNext() {
                            ...
                            while (trnIt.hasNext()) {
                                ...
                                if ((_last == null || trn.getStartToken().compareTo(_last) <= 0) && (first == null || first.compareTo(trn.getEndToken()) <= 0)) {
                                    curNumber = 0;
                                    Token ft = first != null ? first : treenotations.firstToken();
                                    while (ft != null && ft != trn.getStartToken()) {
                                        ft = ft.getNextToken();
                                        curNumber++;
                                    }

                                    if (ft != trn.getStartToken()) {
                                        curNumber = 0;
                                    }
            ...
            for (int j = 0; j < gridWidth; j++) {
                TokenCell tc = grid.get(j);
                coord.setLabelLength((Token) tc.trn, tc.maxLabelLength);
            }

            if (focus != null) {
                double prevDist = coord.scaledDist(cur, focus);
                double newDist = (double) dim.width / (double) (dp.cellWidth * 2);
                focus = null;
                if (prevDist > newDist) {
                    cur = coord.increaseStoragePos(cur, prevDist - newDist);
                    cur = cur < from ? from : cur > to ? to : Math.floor(cur / horStep) * horStep;
                    recountHorScroll();
                }
            }
        }
    ...
    startOfMax = info.host.getEndToken().compareTo(info.slave.getEndToken()) < 0 ? info.slave.getStartToken() : info.host.getStartToken();*/
        ...
        private void dropSingleTrn(Treenotation trn, int base, int height, int startGridShift, int tokenCellCount, int bg_r, int bg_g, int bg_b) throws TreetonModelException {
            ...
            while (true) {
                ...
                if (enoughSpace == false) {
                    ...
                    if (baseCell.trn != trn.getEndToken()) {
                        while (j < startGridShift + tokenCellCount) {
                            TokenCell tc = grid.get(j);
                            if (h < tc.firstFree) {
                                enoughSpace = false;
                ...
                double l = ((double) trn.getEndNumerator()) / trn.getEndDenominator();
                l -= ((double) trn.getStartNumerator()) / trn.getStartDenominator();

                double labelLengthPerSymbol = Math.max(
                        label.label.getPreferredBounds(getGraphics(), fontSmall).getWidth() + 4,
                        caption.label.getPreferredBounds(getGraphics(), fontSmall).getWidth() + 4)
                        / l;
                j = base + startGridShift;
                while (j >= startGridShift) {
                    TokenCell tc = grid.get(j);
                    tc.addLabelLengthPerSymbol(labelLengthPerSymbol);
                    int k = 0;
                    for (; k < height - 1; k++) {
                        if (tc.firstFree == h + k)
                            tc.firstFree++;
        ...
        private Cell getCell(Point p) {
            ...
            for (i = 0; i < gridWidth; i++) {
                ...
                int xStart = (int) Math.floor(coord.scaledDistToStart(cur, (Token) tc.trn) * cellWidth);
                int xEnd = (int) Math.floor(coord.scaledDistToEnd(cur, (Token) tc.trn) * cellWidth);
        ...
        public void mouseClicked(MouseEvent e) {
            ...
            if (e.getButton() == MouseEvent.BUTTON1) {
                ...
                Treenotation trn = newSelection == null ? null : newSelection.trn;
        ...
        public String getToolTipText(MouseEvent e) {
            return ScalableTreenotationViewPanel.this.getToolTipText(e);
        }
        ...
        private void paintGrid(Graphics2D g) {
            ...
            if (gridWidth > 0) {  //начинаем отрисовку сетки
                ...
                int start = tc.trn.toInt(); //начало (в символах) первого токена
                tc = grid.get(gridWidth - 1);
                int end = tc.trn.endToInt(); //конец (в символах) последнего токена

                if (end >= to) { //удостоверимся, что to отображается (занесен в сетку) - иначе нельзя делать coord.storage2scaled(to), да и рисовать пустую область нет смысла
                    fake = coord.scaledDist(cur, to);
                    if (fake > 0) { //рисование пустой области на пространстве справа от самого правого токена хранилища
                ...
                fake = coord.scaledDist(cur, start) * cellWidth;

                int h = dim.height - 2 - bigDescent - (gridIsOn ? gridNumbersHeight : 0);
                double xl = fake; // x1 всегда <= 0
                for (int i = start; i < end; i++) { // рисуем затемненный фон текстового выделения и курсора
                    ...
                    double symWidth = coord.getSymbWidth(i);
                    if (cursor || textSelection) {
                        g.setColor(textSelection ? (cursor ? cursorWithSelectionColor : selectionColor) : cursorColor);
                        g.fillRect((int) xl, 0, (int) (symWidth * cellWidth - 1), dim.height - 1 - additionAreaHeight);
                    }
                    if (gridIsOn) {  // белая вертикальная линяи сетки
                        g.setColor(Color.WHITE);
                        g.drawLine((int) xl, 0, (int) xl, dim.height - 1 - additionAreaHeight);
                    }
                    xl += symWidth * cellWidth;
                }
                if (gridIsOn) { // белая вертикальная линяи сетки
                    g.setColor(Color.WHITE);
                    g.drawLine((int) xl, 0, (int) xl, dim.height - 1 - additionAreaHeight);
                    ...
                    for (int j = 0; j < gridHeight; j++) {
                        ...
                        if (c.trn != null) {
                            ...
                            if (c.background_r != -1) { // Отрисовка фона ячейки
                                ...
                                boolean ns = c.trn != null && !trnIsSolid(c.trn) && c.parent == null;
                            ...
                            if (c.trn == TRN_RELATION) { //отрисовка ячейки под синтаксическую связь   (устаревший код!!!)
                                ...
                                try {
                                    g.setColor(GuiConfiguration.getInstance().getColorForType(c.trn.getType().getName()));
                                } catch (TreetonModelException e) {
                                    g.setColor(Color.RED);
                                }
                ...
                for (int i = 0; i < nRelations; i++) {
                    ...
                    if (relation.info.host == ROOT) {
                        if (relation.info.type != null) {
                            ...
                            int rightEndX = (int) Math.floor(coord.scaledDistToEnd(cur, (Token) tc.trn) * cellWidth);
                            tc = grid.get(relation.leftCellStartX);
                            int leftStartX = (int) Math.floor(coord.scaledDistToStart(cur, (Token) tc.trn) * cellWidth);
                        ...
                        if (relation.leftCellStartX == -1 && relation.rightCellEndX == -1) {
                            tc = grid.get(0);
                            xStart = (int) Math.floor(coord.scaledDistToStart(cur, (Token) tc.trn) * cellWidth);
                            tc = grid.get(gridWidth - 1);
                            xEnd = (int) Math.floor(coord.scaledDistToEnd(cur, (Token) tc.trn) * cellWidth);
                        } else if (relation.leftCellStartX == -1) {
                            tc = grid.get(relation.rightCellStartX);
                            int rightStartX = (int) Math.floor(coord.scaledDistToStart(cur, (Token) tc.trn) * cellWidth);
                            tc = grid.get(relation.rightCellEndX);
                            int rightEndX = (int) Math.floor(coord.scaledDistToEnd(cur, (Token) tc.trn) * cellWidth);
                            ...
                            int leftEndX = (int) Math.floor(coord.scaledDistToEnd(cur, (Token) tc.trn) * cellWidth);
                ...
                xl = fake;
                double x = xl + (coord.getSymbWidth(start) * cellWidth - bigFontWidth) / 2;
                for (int i = start; i < end; i++) {
                    ...
                    if (cursor || textSelection) {
                        ...
                        g.fillRect((int) xl, dim.height - 1 - additionAreaHeight, (int) (symWidth * cellWidth - 1), dim.height - 1);
                    }
                    if (gridIsOn) {
                        g.setColor(Color.WHITE);
                        g.drawLine((int) xl, dim.height - 1 - additionAreaHeight, (int) xl, dim.height - 1);
                    }
                    g.setColor(Color.BLACK);
                    g.drawChars(document, i, 1, (int) x, h);
                    double delta = symWidth * cellWidth;
                    if (i < end - 1)
                        x += (delta + coord.getSymbWidth(i + 1) * cellWidth) / 2;
                    xl += delta;
                }
                if (gridIsOn) {
                    g.setColor(Color.WHITE);
                ...
                if (gridIsOn) { //отрисовка номеров символов
                    ...
                    x = fake + (coord.getSymbWidth(start) * cellWidth - (numbersFontHeight - numbersDescent)) / 2 + numbersFontHeight - numbersDescent;
                    g.setFont(fontNumbers);
                    for (int i = start; i < end; i++) {
                        g.setColor(gridNumbersColor);
                        String s = Integer.toString(i);
                        s.getChars(0, s.length(), spaces, maxOffsetLen - s.length());
                        g.drawChars(spaces, 0, maxOffsetLen, -dim.height + 2, (int) x);
                        Arrays.fill(spaces, maxOffsetLen - s.length(), maxOffsetLen - 1, ' ');
                        if (i < end - 1)
                            x += (coord.getSymbWidth(i) * cellWidth + coord.getSymbWidth(i + 1) * cellWidth) / 2;
                    }
        ...
        public void paint(Graphics _g) {
            ...
            if (!initialized) {
                ...
                numbersFontHeight = fm.getHeight();
                ...
                if (cellWidth != 0) {
                    ...
                    coord.setInitialCellWidth(cellWidth);
                }
        ...
        private class TokenCell extends Cell {
            ...
            public void addLabelLengthPerSymbol(double v) {
                double l = ((double) trn.getEndNumerator()) / trn.getEndDenominator();
                l -= ((double) trn.getStartNumerator()) / trn.getStartDenominator();
                maxLabelLength = Math.max(maxLabelLength, (int) (v * l));
            }
        }
    }
...
/*        Fraction.assign(fakeFraction,(int)(Math.random()*document.length*2),(int)(Math.random()*3+1));
    ...
    public class ScaleButton extends JButton implements MouseListener {
        boolean pressed = false;

        public void paint(Graphics g) {
            Dimension dim = this.getSize();

            g.setColor(pressed ? Color.DARK_GRAY : Color.LIGHT_GRAY);
            g.fillRect(0, 0, dim.width, dim.height);
            g.setColor(pressed ? Color.WHITE : Color.BLACK);

            g.drawLine((int) Math.round((dim.getWidth() - 2) / 4 - 1) + 1, 1, (int) Math.round((dim.getWidth() - 2) / 4 - 1) + 1, dim.height - 2);
            g.drawLine((int) Math.round(dim.getWidth() - 2), 1, (int) Math.round(dim.getWidth() - 2), dim.height - 2);
            g.drawLine((int) Math.round((dim.getWidth() - 2) / 4 - 1) + 1, dim.height / 2, (int) Math.round(dim.getWidth() - 2), dim.height / 2);
        }
        ...
        public void mousePressed(MouseEvent e) {
            if (!isEnabled())
                return;
            pressed = true;
            dp.coordConverterNum = (dp.coordConverterNum + 1) % dp.coordConverters.length;
            int initialCellWidth = dp.coord.getInitialCellWidth();
            dp.coord = dp.coordConverters[dp.coordConverterNum];
            dp.coord.setInitialCellWidth(initialCellWidth);
            componentResized(null);
            dp.repaint();
        }
